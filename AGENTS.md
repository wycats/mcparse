...
- **Finalize**: Once the user has approved the walkthrough:
  ...
  - **Commit**: Commit the changes to the repository to preserve the history of the `docs/agent-context/current/` files. **CRITICAL**: This commit MUST happen BEFORE you empty out or update the `docs/agent-context/current/` files for the next phase. The commit should capture the final state of the completed phase.
  - **Prepare for Next Phase**: ONLY AFTER the commit is made, empty out the files in `docs/agent-context/current/` directory to prepare for the next phase.
